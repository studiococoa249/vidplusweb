...
export default async function DramaShortApp({ params }: { params: Promise<{ lang: string }> }) {
  ...
  // Create dummy categories by slicing the fetched data (since we don't have deep genre categorization yet)
                ...
                return (
                  <Link
                    ...
                    className={`relative block rounded-2xl overflow-hidden group cursor-pointer
                      ${isLarge ? 'col-span-2 row-span-1 md:row-span-2' : 'col-span-1 row-span-1'}
                      ${hiddenMobile ? 'hidden md:block' : ''}
                      ${hiddenTablet ? 'hidden lg:block' : ''}
                    `}
                  >
                    <img src={item.banner_url || `https://placehold.co/400x600/172033/fff?text=${encodeURIComponent(item.name)}`} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
          ...
          <section id="romantis" className="mb-14">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {loveData.map((item, index) => (
                <Link href={`/${lang}/play/${item.id}`} key={item.id} className="bg-[#121622] border border-gray-800 rounded-2xl p-3 flex gap-4 items-center group cursor-pointer hover:bg-[#1c2235] hover:border-pink-500/30 transition duration-300">
                  <div className="w-20 h-28 md:w-24 md:h-32 rounded-xl overflow-hidden flex-shrink-0 relative">
                    <img src={item.banner_url || `https://placehold.co/300x400/4c1d95/f9a8d4?text=Love+${index + 1}`} alt={item.name} className="w-full h-full object-cover group-hover:scale-110 transition duration-500" />
              ...
              {actionData.map((item, index) => (
                ...
                >
                  <div className="relative aspect-[3/4] rounded-2xl overflow-hidden mb-3">
                    <img src={item.banner_url || `https://placehold.co/400x533/171717/ef4444?text=Action+${index + 1}`} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" />
