...
import { updateShortDrama } from "../../../../actions";
...
export default function EditForm({
  ...
  initialData: ShortDrama;
  genres: Genre[];
}) {
  const [state, formAction, isPending] = useActionState(updateShortDrama, null);
  const [slug, setSlug] = useState(initialData.slug);
  const params = useParams();
  const lang = (params.lang as string) || "id";

  const selectedGenres = new Set(initialData.id_genre || []);
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Edit Drama Pendek</h1>
          <p className="text-gray-400 text-sm">Perbarui data serial drama</p>
        </div>
      </div>
        ...
        <form action={formAction} className="space-y-6">
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-300">
                  Judul Drama <span className="text-red-400">*</span>
                </label>
                <input
                  type="text"
                  name="name"
              ...
              <div className="space-y-2">
                ...
                {initialData.banner_url && (
                  <img
                    ...
                    alt="Banner preview"
                    className="mt-2 h-32 rounded-lg object-cover border border-gray-700"
                  />
                )}
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  type="number"
                  name="total_episode"
                  defaultValue={initialData.total_episode}
                  min="0"
                  className="w-full bg-[#0a0c13] border border-gray-700 rounded-xl py-2.5 px-4 text-white focus:outline-none focus:border-[#ffbd59] focus:ring-1 focus:ring-[#ffbd59]/50 transition-all"
                />
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-gray-300">
                  Pilih Genre (Bisa Lebih Dari Satu)
                </label>
                <div className="bg-[#0a0c13] border border-gray-700 rounded-xl p-4 max-h-48 overflow-y-auto space-y-2">
                  {genres.length > 0 ? (
                    ...
                    <p className="text-sm text-gray-500">
                      Belum ada genre. Silakan buat genre terlebih dahulu.
                    </p>
                  )}
                </div>
              </div>
