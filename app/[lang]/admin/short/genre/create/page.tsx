...
export default function CreateGenrePage() {
  const [state, formAction, isPending] = useActionState(createGenre, null);
    ...
    <div className="w-full space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">Buat kategori genre baru</p>
        </div>
      </div>
        ...
        <form action={formAction} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <input 
              ...
              className="w-full bg-[#0a0c13] border border-gray-700 rounded-xl py-2.5 px-4 text-white focus:outline-none focus:border-[#ffbd59] focus:ring-1 focus:ring-[#ffbd59]/50 transition-all"
            />
            <p className="text-xs text-gray-500">Slug harus unik dan tidak boleh mengandung spasi (gunakan tanda hubung -).</p>
          </div>
          ...
          <div className="pt-4 flex justify-end gap-3">
            ...
            >
              <Save size={18} />
              <span>{isPending ? 'Menyimpan...' : 'Simpan Genre'}</span>
            </button>
          </div>
        </form>
      </div>
