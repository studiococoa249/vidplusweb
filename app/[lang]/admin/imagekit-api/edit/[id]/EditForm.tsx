...
export default function EditForm({ initialData }: { initialData: any }) {
  const [state, formAction, isPending] = useActionState(updateImageKit, null);
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">Edit ImageKit API</h1>
          <p className="text-gray-400 text-sm">Perbarui kredensial API ImageKit</p>
        </div>
      </div>
        ...
        <form action={formAction} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <input 
              type="url" 
              name="url_endpoint"
              defaultValue={initialData.url_endpoint}
              required
              className="w-full bg-[#0a0c13] border border-gray-700 rounded-xl py-2.5 px-4 text-white font-mono text-sm focus:outline-none focus:border-[#ffbd59] focus:ring-1 focus:ring-[#ffbd59]/50 transition-all"
            />
          </div>
