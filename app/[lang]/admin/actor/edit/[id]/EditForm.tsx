...
export default function EditForm({ initialData }: { initialData: any }) {
  const [state, formAction, isPending] = useActionState(updateActor, null);
  const params = useParams();
  const lang = (params.lang as string) || "id";
  const [slug, setSlug] = useState<string>(initialData.slug ?? "");
  const [slugManual, setSlugManual] = useState(true);
  const [bannerUrl, setBannerUrl] = useState<string>(initialData.actor_banner_imagekit_url ?? "");
    ...
    <div className="w-full space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-gray-400 text-sm">Perbarui data actor</p>
        </div>
      </div>
        ...
        <form action={formAction} className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              className="w-full bg-[#0a0c13] border border-gray-700 rounded-xl py-2.5 px-4 text-white focus:outline-none focus:border-[#ffbd59] focus:ring-1 focus:ring-[#ffbd59]/50 transition-all font-mono text-sm"
            />
